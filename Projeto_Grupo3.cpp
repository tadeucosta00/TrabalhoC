...
void inserirMembros(char *password, char *passwordgestor,char *passwordequipa)
{
 	...
 	do
	{
		...
		// inserir o membro á equipa selecionada
		...
		// Guardar informações do membro e a sua profissão
		ID_membro++;
	...
	{
		// Gerir membros apenas se tiver um profissional de cada área
		if(contadormed1[id] >= 1 && contadorenf1[id] >= 1 && contadorass1[id] >= 1 && contadortec1[id] >= 1)
		{
			gerirMembros(password,passwordgestor,passwordequipa);
		}
		// Caso não tenho um profissional de cada área vai para o menu de inserir membros
		else
		{
			inserirMembros(password,passwordgestor,passwordequipa);
			printf("\n\nA equipa tem obrigatoriamente de ter um profissional de cada área!");
		}
	}	
}
...
void painelchefeequipa(char *password,char *passwordequipa,char *passwordgestor)
{
	...
	do
	{
		...
		printf("Digite o seu ID se for capitão, se não for porfavor insira 500 para voltar atras: ");
		scanf("%d", &id);
...
void ListarVisitas(char *password,char *passwordequipa,char *passwordgestor)
{
	...
	for (i = 0; i < id_visita; i++)
	{
		...
		printf("%dº Lar %d - %d", i+1, id, maior);
	}
}
